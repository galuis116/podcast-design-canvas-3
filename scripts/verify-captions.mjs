...
// Drives the shipped app in headless Chrome and proves the caption import
// workflow end to end: upload two generated speaker WebM videos through the
// normal Host/Guest controls, upload a generated WebVTT with two timed cues,
// verify captions appear only inside their cue ranges during playback and while
// scrubbing, confirm captions persist across Split/Stack/Spotlight, then export
// and confirm caption overlays are burned into decoded frames at matching times.
...
(async () => {
  ...
  await waitFor(() => window.PDC && window.PDC.captions && document.querySelector("#caption-file")
    && document.querySelector("#export") && document.querySelector("#scrub"), "shipped caption/scrub/export controls should exist");
  ...
  await waitFor(
    ...
    "uploaded speakers should decode with a real duration covering both caption ranges", 400,
  );
